...
# Подсказка по типу обращения, добавляемая к поисковому запросу.
# Одного слова («запись») мало: в смешанном обращении лексика второго
# намерения перевешивает, и раздел про запись не попадает в топ-3.
# Проверено на реальных формулировках — см. tests/test_retrieve.py.
TYPE_HINTS: dict[str, str] = {
    "справка": "выдача справки, перечень документов, срок изготовления",
    "жалоба": "жалоба на действия работника, порядок подачи, срок рассмотрения",
    "запись": "запись на приём, талон, время приёма, перенос записи",
}


def build_query(request_type: str, text: str) -> str:
    """Поисковый запрос: подсказка по типу обращения плюс сам текст."""
    hint = TYPE_HINTS.get(request_type, "")
    return f"{hint}. {text.strip()}" if hint else text.strip()
